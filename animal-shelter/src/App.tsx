  ...
  return <>
    ...
    <UserContainer.Provider value={{ user: user, setUser: setUser, forceUpdateUser: apiCall }}>
      <PetsContainer.Provider value={{ pets: pets ? pets : [], setPets: setPets }}>
        ...
        <dialog ref={loginRef} style={{ border: "none", borderRadius: "24px", minWidth: "300px", width: "fit-content" }}>
              ...
              <div className="adopt-form-container">
                ...
                <form id="adopt-form" onSubmit={(e) => {
                  ...
                  userApi.userAuth({
                    ...
                    .then(r => {
                      if (r.status >= 400) {
                        r.text().then((err: string) => {
                          if (err.includes) {
                            err.includes("bad credentials") &&
                              setLoginError("Ошибочная комбинация логин/пароль");
                          }
                          else {
                        ...
                        r.json()
                          .then((j: any) => {
                            Promise.all([
                              ...
                              .then(responses => {
                                alert(`Вы успешно вошли в аккаунт ${responses[0].userName ?? ""}`)
                                setUser({
                                  ...responses[0],
                                  userActions: responses[1] ?? []
                                });
                ...
                <button onClick={() => setIsLogin(false)} style={{ backgroundColor: "rgb(240, 240, 240)", border: "none", marginTop: "7px" }}>Создать аккаунт</button>
              </div>
            </> : <>
              <div className="adopt-form-container">
                        ...
                        r.json()
                          .then((j: any) => {
                            Promise.all([
                              ...
                              .then(responses => {
                                alert(`Успешно создан аккаунт ${responses[0].userName ?? ""}`)
                                setUser({
                                  ...responses[0],
                                  userActions: responses[1] ?? []
                                });
                ...
                <button onClick={() => setIsLogin(true)} style={{ backgroundColor: "rgb(240, 240, 240)", border: "none", marginTop: "7px" }}>Войти в аккаунт</button>
              </div>
            </>}
          </div>
        </dialog>
